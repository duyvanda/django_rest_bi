...
export const MYVNPProvider = ({ children }) => {
  ...
  // Post and Update KH
  const post_data_cn = async (data) => {
    SetLoading(true);
    const response = await fetch(`${URL}/create_cn/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const post_data_cn_update = async (data) => {
    SetLoading(true)
    const response = await fetch(`${URL}/update_cn/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
